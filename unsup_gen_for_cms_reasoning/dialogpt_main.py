...
1. 
    origin:
    -> target1, target2, target3 = model({input of model})
    modification:
    -> tmp = model({input of model})
    -> target1, target2, target3 = tmp[0], tmp[1], tmp[2] 
2. 
    origin:
    -> model(input_ids = {ids}, past={past})
    modification:
    -> model(input_ids = {ids}, past_key_values={past})
3.
    origin:
    -> from transformers.models.modeling_gpt2 import GPT2LMHeadModel
    modification:
    ...
    for i in range(length):
        ...
        if past is None and output_so_far is not None:
            ...
            if output_so_far.shape[1] > 1:
                tmp = model(output_so_far[:, :-1])
                _, past, _ = tmp[0], tmp[1], tmp[2]
                o1_past = past
        tmp = model(past_key_values=past, inputs_embeds=last_embeds)
        unpert_logits, past, unpert_all_hidden = tmp[0], tmp[1], tmp[2]
    ...
    for i in range(max_length):
        ...
        if past is None:
            ...
            if o1_logits.shape[1] > 1:
                tmp = model(inputs_embeds=o1_embeds[:, :-1, :])
                _, past, _ = tmp[0], tmp[1], tmp[2]
...
def backward_pass(
    ...
    for i in range(num_backward_iters):
        ...
        tmp = model(inputs_embeds=inputs_embeds)
        all_logits, _, _ = tmp[0], tmp[1], tmp[2]
